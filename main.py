...
# URL pública do modelo no Supabase Storage
...
@app.get("/")
def health():
    return {"status": "ok", "servico": "LabCQ Excel API v2"}

@app.post("/gerar-excel")
async def gerar_excel(payload: PayloadExcel):
    COLUNAS = ['C','D','E','F','G','H','I','J','K']

    # Baixa modelo do Supabase Storage
    async with httpx.AsyncClient() as client:
        resp = await client.get(MODELO_URL)
        if resp.status_code != 200:
            return {"erro": f"Modelo não encontrado: {resp.status_code}"}
        modelo_bytes = resp.content
